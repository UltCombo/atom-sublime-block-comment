...
export default {
  ...
  toggle() {
    ...
    buffer.transact(() => {
      for (let i in editor.getSelectionsOrderedByBufferPosition().reverse()) {
        const selection = editor.getSelectionsOrderedByBufferPosition().reverse()[i];
        const langs = editor.scopeDescriptorForBufferPosition(selection.getBufferRange().start).getScopesArray();
        ...
        for (let lang of langs) {
          if (lang.indexOf('text.html') > -1) {
            commentTokens = {
              open:  '<!--',
              close: '-->',
            };
          }
          if ((lang.indexOf('source.js.embedded.html') > -1) ||
              (lang.indexOf('source.css.embedded.html') > -1)) {
                commentTokens = {
                  open:  '/*',
                  close: '*/',
                };
