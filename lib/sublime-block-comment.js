...
export default {
  ...
  toggle() {
    ...
    buffer.transact(() => {
      for (let selection of editor.getSelectionsOrderedByBufferPosition().reverse()) {
        const range = selection.getBufferRange();
        const langs = getPositionScopes(range.start);
        // TODO change comment syntax based on scope's grammar
        let commentTokens = {
          open:  '/*',
          close: '*/'
        };
        if (lang[0].includes('text.html')) {
          commentTokens = {
            open:  '<!--',
            close: '-->'
          };
        }
        if (lang[1].includes('source.js.embedded.html') ||
            lang[1].includes('source.css.embedded.html')) {
              commentTokens = {
                open:  '/*',
                close: '*/'
              };
